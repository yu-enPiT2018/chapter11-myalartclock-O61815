...
class SimpleAlertDialog : DialogFragment(){
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?) : Dialog {
        ...
        val builder = AlertDialog.Builder(context).apply {
            setMessage("ASA-desu")
            setPositiveButton("OKIRU"){
                dialog,which -> context.toast("OKIRUgaKURIKKUsaremasite")
            }
            setNegativeButton("ato5FUN"){
                dialog,which -> context.toast("ato5FUNgaKURIKKUsaremasite")
            }
        }
...
class TimePickFragment : DialogFragment() ,
        TimePickFragment.OnTimeSectedListener{
    ...
    override fun onAttach(context: Context?) {
        super.onAttach(context)
        if (context is TimePickFragment.OnTimeSelectedListener) {
            listener = context
        }
    }
    override fun onCreateDialog(savedInstanceState: Bundle?):Dialog{
